...
void EntityManager::EntityCollisionCheck()
{
	...
	for (auto itr = m_entities.begin(); std::next(itr) != m_entities.end(); ++itr)
	{
		for (auto itr2 = std::next(itr); itr2 != m_entities.end(); ++itr2)
		{
			...
			// … inside your double‐for loop, after the normal AABB check …

			EntityType a = itr->second->GetType();
			EntityType b = itr2->second->GetType();

			// only Player attacking Enemy
			if (a == EntityType::Player && b == EntityType::Enemy)
			{
				auto *player = static_cast<Character *>(itr->second);
				player->UpdateAttackAABB(); // make sure the box is up to date
				if (player->GetState() == EntityState::Attacking &&
					player->m_attackAABB.intersects(itr2->second->m_AABB))
				{
					player->OnEntityCollision(itr2->second, true);
				}
			}
			// only Enemy attacking Player
			else if (a == EntityType::Enemy && b == EntityType::Player)
			{
				auto *enemy = static_cast<Character *>(itr->second);
				enemy->UpdateAttackAABB();
				if (enemy->GetState() == EntityState::Attacking &&
					enemy->m_attackAABB.intersects(itr2->second->m_AABB))
				{
					enemy->OnEntityCollision(itr2->second, true);
				}
			}
		}
	}
