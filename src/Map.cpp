...
void Map::LoadMap(const std::string &l_path)
{
	...
	while (std::getline(mapFile, line))
	{
		...
		else if (type == "PLAYER")
		{
			...
			// Check all required resources before setting up HUD
			bool resourcesLoaded = true;
			resourcesLoaded &= m_context->m_textureManager->RequireResource("HeartFull");
			resourcesLoaded &= m_context->m_textureManager->RequireResource("HeartEmpty");
			resourcesLoaded &= m_context->m_textureManager->RequireResource("Hud");
			resourcesLoaded &= m_context->m_textureManager->RequireResource("HudItems1"); // Changed from "GoldCoin" to match textures.cfg

			if (!resourcesLoaded)
			{
				std::cout << "! Failed loading required HUD resources" << std::endl;
				return;
			}

			// Setup only necessary HUD elements
			m_items.resize(1);	// Now works with vectors
			m_labels.resize(1); // Now works with vectors
			...
			// Setup coin sprite
			sf::Texture *coinTexture = m_context->m_textureManager->GetResource("HudItems1");
			if (coinTexture)
			{
				m_items[0].setTexture(*coinTexture);
				m_items[0].setTextureRect(sf::IntRect(0, 0, 16, 16));
			}

			// Setup font
			if (!m_font.loadFromFile(Utils::GetResourceDirectory() + "media/Fonts/arial.ttf"))
			{
				std::cout << "! Failed loading font" << std::endl;
				return;
			}
			m_labels[0].setFont(m_font);
			m_labels[0].setCharacterSize(12);
...
void Map::LoadBackGround(const std::string &l_path)
{
	...
	if (!mapFile.is_open())
	{
		std::cout << "! Failed loading background map file: " << l_path << std::endl;
		return;
	}
	EntityManager *entityMgr = m_context->m_entityManager;
	std::string line;
...
void Map::Draw()
{
	...
	// Items HUD
	sf::Vector2u windowSize = l_wind->getSize();
	sf::Vector2f viewSize = l_wind->getView().getSize();
	sf::Vector2f viewCenter = l_wind->getView().getCenter();

	// Only set up the gold coin item
	sf::Vector2f itemBoxPosition(
		viewCenter.x - (viewSize.x / 2) + 16,
		viewCenter.y - (viewSize.y / 2) + 16);

	m_items[0].setPosition(itemBoxPosition);
	m_labels[0].setPosition(itemBoxPosition.x + 15, itemBoxPosition.y + 1);

	m_labels[0].setString(sf::String(std::to_string(m_context->m_gold)));

	// Only draw the gold coin item
	l_wind->draw(m_items[0]);
	l_wind->draw(m_labels[0]);
}
...
void Map::PurgeMap()
{
	...
	m_context->m_textureManager->ReleaseResource("HudItems1"); // Changed from "GoldCoin"
}
