...
{
	// assume m_position is bottom‑center of sprite
	float dir = (m_spriteSheet.GetDirection() == Direction::Left) ? -1.f : +1.f;
	m_attackAABB.left = m_position.x + dir * m_attackAABBoffset.x;
	m_attackAABB.top = m_position.y - m_attackAABBoffset.y;
	// width & height already read from file
}
